  ...
  <div class="flex flex-col h-dvh w-dvw items-center justify-center bg-white">
    ...
    >
      <div class="flex flex-col space-y-1.5 p-6 text-center">
        <h3 class="font-semibold tracking-tight text-2xl">Create New Account</h3>
      </div>
      <div class="p-6 pt-0">
        <Form
        ...
        >
          ...
          <div class="space-y-1">
            ...
            <ul class="mt-2 space-y-1 text-sm">
              <li :class="{ 'text-green-600': hasMinLength, 'text-red-500': !hasMinLength }">
                {{ hasMinLength ? '✓' : '•' }} At least 8 characters
              </li>
              <li :class="{ 'text-green-600': hasLetter, 'text-red-500': !hasLetter }">
                {{ hasLetter ? '✓' : '•' }} At least one letter
              </li>
              <li :class="{ 'text-green-600': hasNumber, 'text-red-500': !hasNumber }">
          ...
          <button
            ...
            class="cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#18181b] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 bg-[#18181b] text-[#fafafa] shadow hover:bg-[#18181b]/90 h-9 px-4 py-2 w-full"
          >
            {{ isSubmitting ? 'Creating...' : 'Create an account' }}
          </button>
          <p v-if="apiError" class="text-red-500 text-center">{{ apiError }}</p>
          <p class="text-center">
            <span class='text-zinc-700'>Already have an account?</span>
            <router-link to="/auth/login" class='font-bold text-zinc-900'>Login</router-link>
          </p>
        </Form>
      </div>
    </div>
